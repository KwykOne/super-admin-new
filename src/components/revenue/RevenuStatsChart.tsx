  ...
  const fetchSellerStats = useCallback(async () => {
    try {
      ...
      const res = await axios.get(
        `${baseURL}api/v1/admin/revenue/revenue-stats?date=${period}&is_test=${dataType}`,
        {
          headers: {
    ...
    (newPeriod: PeriodType, customDateRange?: DateRange) => {
      ...
      if (
        ...
        filteredData = sellerStatsData.map((item: any) => ({
          date: item.date,
          [metricName]: parseInt(item.revenue),
        }));
        ...
        filteredData = daysOfWeek.map((day) => {
          ...
          return {
            ...day,
            [metricName]: match ? parseInt(match.revenue) : 0,  // Set the count
          };
        });
